...
import 'package:social/repositories/auth_repository.dart';
...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onLogin(OnLoginEvent event, Emitter<AuthState> emit) async {
    try {
      ...
      final data = await authRepositories.login(event.email, event.password);
  ...
  Future<void> _onCheckingLogin(
    ...
    try {
      ...
      if (await secureStorage.readToken() != null) {
        final data = await authRepositories.renewLogin();
