...
class _ChatMessagesPageState extends State<ChatMessagesPage>
  ...
  _historyMessages() async {
    final List<ListMessage> list =
        await chatRepositories.listMessagesByUser(widget.uidUserTarget);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          CircleAvatar(
            backgroundImage: NetworkImage(UA.baseUrl + widget.avatarTarget),
          ),
          ...
          BlocBuilder<ChatBloc, ChatState>(
            builder: (_, state) => TextButton(
                onPressed: state.isWritting
                    ...
                    : null,
                child: const TextCustom(text: 'Send', color: ColorsSM.primary)),
          )
        ],
      ),
    );
